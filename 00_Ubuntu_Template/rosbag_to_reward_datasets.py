...
# selected set, "expert" is the higher-ranked half, "medium" is the lower-ranked
...
EPISODE_RANKING_METRIC_BY_REWARD = {
    # V1 rewards are mostly negative, so total return favors tiny episodes.
    # Rank V1 by longest runtime instead.
    "physical_reward_v1": "duration_s",
}
...
def episode_ranking_metric_for_reward(reward_name: str) -> str:
    return str(
        EPISODE_RANKING_METRIC_BY_REWARD.get(reward_name, EPISODE_RANKING_METRIC)
    ).strip().lower()
...
def rank_episode_summaries(
    ...
    *,
    ...
    valid_metrics = {"return", "mean_reward", "num_transitions", "duration_s"}
    if metric not in valid_metrics:
        raise ValueError(
            "EPISODE_RANKING_METRIC must be one of: "
            + ", ".join(sorted(valid_metrics))
        )
    def score(item: dict[str, Any]) -> tuple[float, int]:
        value = item.get(metric)
        metric_value = float(value) if value is not None and np.isfinite(float(value)) else 0.0
        return metric_value, int(item["num_transitions"])
...
def split_top_episode_transitions(
    ...
    *,
    ...
    ranking_metric = episode_ranking_metric_for_reward(reward_name)
    ranked = rank_episode_summaries(summaries, metric=ranking_metric)
    if not ranked:
